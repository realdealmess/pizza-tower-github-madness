if (state != enemy_states.grabbed)
{
    with (obj_player)
    {
        ...
        if ((attacking == 0) && (((state != states.tackle) && (state != states.hurt)) && ((!(y < other.y)) && ((grabbing == 0) && (other.state != enemy_states.stun)))))
        {
            if (x != other.x)
            {
                other.image_xscale = (-sign((other.x - x)))
            ...
            other.vsp = -4
            if ((other.state == enemy_states.walk) || (other.state == enemy_states.turn))
                other.state = enemy_states.idle
            image_index = 0
            state = states.bump
            if (other.state == enemy_states.land)
                other.state = enemy_states.idle
        }
    }
}
